...
import { FileTransfer, User, TransferStatus } from '../../lib/types/file-transfer'
...
function getStatusInfo(status: TransferStatus) {
  const statusMap: Record<TransferStatus, { label: string; badgeClass: string }> = {
    ...
    failed: { label: '失败', badgeClass: 'bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400' }
  }
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between mb-1">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <span className={cn('px-2 py-0.5 text-xs font-medium rounded', statusInfo.badgeClass)}>
                {statusInfo.label}
              </span>
            </div>
          ...
          <div className="flex items-center gap-2 mt-2">
            ...
            {['waiting', 'transferring', 'paused'].includes(transfer.status) && onCancel && (
              <button
                onClick={onCancel}
                className="px-3 py-1 text-sm text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200 transition-colors"
              >
                取消
              </button>
            )}
